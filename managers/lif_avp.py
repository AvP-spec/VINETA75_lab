...
class LIFManager(TerminalColours):
    ...
    def __init__(self, silent=True):
        ...
        ## variables for temperature control in lif_monitor
        self.t_steps = [0.01, 0.05, 0.1, 0.5, 1.0]
        self._t_step_idx = 2 # defoult temperature step 0.1 °C
        self._control_mode = 'piezo' # 'piezo' or 'temperature', toggled with keys 1/2
        self._active_plotter = None # reference to plotter, set in lif_monitor()
    ...
    def check_temperature_limits(self, value: float) -> float | None:
        limits = self.master_diode.temperature['limits']
        if value < limits['min'] or value > limits['max']:
            print(f"{self.RED} Error in check_temperature_limits() from lif_avp.py: {self.RESET}")
            print(f"Temperature {value:.4f} °C is out of limits: "
                  f"[{limits['min']}, {limits['max']}]")
            print("value is skipped.")
            return None
    ...
    def lif_monitor(self, max_points=100, 
        ...
        ## initialize setpoint reference line / mode text with current state
        try:
            plotter.set_temp_setpoint(float(self.master_diode.read_settemperature(silent=True)))
        except Exception as e:
            print(f"{self.RED}Could not read initial temperature setpoint: {e}{self.RESET}")
        plotter.set_mode_text(self._control_mode)

        self._monitoring_active = True
        print(f"{self.GREEN}Monitoring started.{self.RESET}")
        print(f"----- Controls ------")
        print(f"control mode piezo/temperature: {self.CYAN}1 / 2 {self.RESET}")
        print(f"active value increse/decrease: {self.CYAN}UP/DOWN {self.RESET}")
        print(f"piezo voltage steps: {self.v_steps}V, "
              f"defoult step: {self.v_steps[self._step_idx]}V")
        print(f"temperature steps: {self.t_steps}°C, "
              f"defoult step: {self.t_steps[self._t_step_idx]}°C")
        print(f"step size of active mode: {self.CYAN}LEFT/RIGHT {self.RESET}")
        print(f"Clear plot / zoom in on current values: {self.CYAN} c {self.RESET}")
        ...
        try:
            while self._monitoring_active:
                ...
                temperature = self.master_diode.read_temperature(silent=True)
                wavelength = self.wlm.read(
                                            device_read_time=False,
                                            slow=False,
                                            silent=True
    ...
    def _on_key(self, event):
        ...
        # Clear all plotted points and reset the rolling time window,
        # e.g. to "zoom in" once a target value has been reached manually
        if event.key == 'c':
            if self._active_plotter is not None:
                self._active_plotter.clear_data()
                print(f"{self.CYAN}Plot cleared.{self.RESET}")
            return

        # Handle control mode switching (1 = piezo, 2 = temperature)
        if event.key in ['1', '2']:
            self._control_mode = 'piezo' if event.key == '1' else 'temperature'
            print(f"Control mode -> {self.CYAN}{self._control_mode}{self.RESET}")
            if self._active_plotter is not None:
                self._active_plotter.set_mode_text(self._control_mode)
            return

        # Handle value changes (UP/DOWN arrows), depends on active control mode
        if event.key in ['up', 'down']:
            if self._control_mode == 'piezo':
            ...
            elif self._control_mode == 'temperature':
                step = self.t_steps[self._t_step_idx]
                set_temp_str = self.master_diode.read_settemperature(silent=True)
                set_temp = float(set_temp_str)
                target_t = set_temp + (step if event.key == 'up' else -step)

                validated_t = self.check_temperature_limits(target_t)
                if validated_t is not None:
                    t = validated_t
                    self.master_diode.set_temperature(value=t, unit="C", silent=True)
                    print(f"Temperature -> {t:.4f} °C (step: {step})")
                    if self._active_plotter is not None:
                        self._active_plotter.set_temp_setpoint(t)
                    ## short pause, no full wait_for_temperature: the plot shows
                    ## the actual temperature settling live, the loop keeps running
                    time.sleep(1.5)

        # Handle step size changes (LEFT/RIGHT arrows), for the active control mode
            ...
            if self._control_mode == 'piezo':
                ...
                print(f"Current piezo step size: {self.v_steps[self._step_idx]} V")
            elif self._control_mode == 'temperature':
                # Cycle through the predefined t_steps list
                self._t_step_idx = (self._t_step_idx + direction) % len(self.t_steps)
                print(f"Current temperature step size: {self.t_steps[self._t_step_idx]} °C")
...
class PiezoScanLifePlotter(PlotStyler, TerminalColours):
    ...
    def _setup_axes(self, v_list):
        ...
        self.ax_wlm.axhline(667.91247, color='gray', linestyle='--', linewidth=2, alpha=0.7)
    ...
    def final_message(self):
        ...
        try:
            ...
            print(f"{self.YELLOW}Warning: set_window_on_top() not available, "
                  f"skipping window-on-top styling.{self.RESET}")
        plt.ioff()
        plt.show(block=True)
...
class LifeLIFPlotter(PlotStyler, TerminalColours):
    ...
    def __init__(self, max_points=100, 
        ...
        ## offset subtracted from incoming 'time_s' values, so that
        ## clear_data() can reset the rolling window back to t=0
        ## without needing to know about LIFManager's absolute start_time
        ...
        ## reference line for the current temperature setpoint,
        ## created lazily on the first call of set_temp_setpoint()
        self.temp_setpoint_line = None

        ## text indicating which control mode (piezo/temperature) is
        ## currently active for the UP/DOWN arrow keys, see set_mode_text()
        self.mode_text = self.fig.text(
                            ...
                            bbox=dict(facecolor='white', alpha=0.8, 
                                      edgecolor='tab:purple', boxstyle='round,pad=0.3')
                            )
    ...
    def clear_data(self):
        """
        Clear all buffered data points and reset the rolling time window
        back to t=0. Useful once a target wavelength/temperature has been
        reached manually: clearing removes the old, off-target points so
        the plot autoscales ("zooms in") to the new points only.
        Triggered by pressing 'c' during lif_monitor(), see LIFManager._on_key().
        """
        last_t = self.data_bufers.get('time_s')
        if last_t:
            ## shift the offset so the *next* point starts back near t=0
            self.time_offset += last_t[-1]

        for buf in self.data_bufers.values():
            buf.clear()
        ...
        ## reset rolling x-window back to the initial view
        self.current_xlim = self.window_width
        self.ax_piezo.set_xlim(0, self.current_xlim)
    ...
    def set_temp_setpoint(self, value: float):
        """
        Draw/update a dashed horizontal reference line in the temperature
        subplot showing the current temperature setpoint. Called whenever
        the setpoint changes (see LIFManager._on_key()), not on every
        data update, since it only changes on key press.
        """
        if self.temp_setpoint_line is None:
            self.temp_setpoint_line = self.ax_temp.axhline(
                                            value, color='gray', 
                                            linestyle='--', linewidth=1.5, 
                                            alpha=0.7, label='Setpoint [°C]'
                                            )
            ## re-draw legend so the new line shows up
            self.ax_temp.legend(loc='upper right')
        else:
            self.temp_setpoint_line.set_ydata([value, value])

        self.ax_temp.relim()
        self.ax_temp.autoscale_view(scalex=False)
        self.fig.canvas.draw_idle()
        self.fig.canvas.flush_events()
    ...
    def set_mode_text(self, mode: str):
        """
        Update the on-figure text indicating which control mode
        (piezo/temperature) is currently active for the UP/DOWN arrow keys.
        """
        label = {"piezo": "Steuerung: Piezo [1]", 
                  "temperature": "Steuerung: Temperatur [2]"}.get(mode, mode)
        color = {"piezo": "tab:purple", 
                  "temperature": "tab:orange"}.get(mode, "black")

        self.mode_text.set_text(label)
        self.mode_text.set_color(color)
        self.mode_text.set_bbox(dict(facecolor='white', alpha=0.8, 
...
if __name__ == "__main__":
    ...
    def test_lif_plotter(read_out_time=0.1):
        ...
        plotter.set_temp_setpoint(16.0)
        plotter.set_mode_text('piezo')
        # plt.pause(0.1)
